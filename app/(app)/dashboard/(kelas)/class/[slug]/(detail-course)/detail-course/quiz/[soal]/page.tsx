...
function Quiz({ params }: { params: { soal: string } }) {
   ...
   function handleAnswer(val: number) {
      const temp = answerButton.find((item, idx) => item.value === val)
      if (temp?.value === answer) {

         setQ(temp.value)
      } else {
         setQ(temp?.value as number)
      }
   }
                     ...
                     return (
                        <button
                           ...
                           className={cn(
                              ...
                              q === item.value ? "border-[#0F973D] shadow-[#0F973D]" : "border-[#DB2323] shadow-[#DB2323]"
                              // ans === item.value && "border-[#0F973D] shadow-[#0F973D]",
                              // select === item.value ? "border-[#0F973D] shadow-[#0F973D]" : "border-[#DB2323] shadow-[#DB2323]"
                           )}>
                           {item.title}
